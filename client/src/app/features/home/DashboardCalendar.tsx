                        ...
                        <div className="mt-4">
                            <h3 className="text-sm font-medium border-b pb-2 mb-2">
                                {value instanceof Date
                                    ? value.toLocaleDateString('pl-PL', { weekday: 'long', day: 'numeric', month: 'long' }) : 'Wybrany zakres'}
                            </h3>
                            ...
                            {selectedDateBills.length > 0 ? (
                                <div className="space-y-2">
                                    {selectedDateBills.map((bill: Bill) => (
                                        <div key={bill.id}
                                            className="flex justify-between items-center text-sm p-2 bg-base-200 rounded-lg">
                                            <div>
                                                <div className="font-bold">{bill.title}</div>
                                                <div className="text-xs opacity-70">
                                                    Termin: {new Date(bill.dueDate).toLocaleDateString()}
                                                </div>
                                            </div>
